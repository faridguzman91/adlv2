(function () {
  ...
  function init() {
    ...
    for (var i = 0; i < numUnits; i++) {
      ...
      else if (units[i].images && units[i].images.length)
        li += '<img src="./img/' + units[i].images[0] + '">';
      li += "</div>";
      li += '<div class="details">';
      li += '<span data-status="' + units[i].status + '">' + status + "</span>";
      li +=
        "<h3>" +
        formatProperty(units[i].name, "name") +
        " " +
        formatProperty(units[i].id, "id") +
        "</h3>";
      li +=
        '<span class="price">' +
        formatProperty(units[i].price, "price") +
        "</span>";
      var lbl = (parseInt(units[i].roomcount) || 0) == 1 ? "kamer" : "kamers";
      li +=
        '<div class="info"><span>' +
        formatProperty(units[i].roomcount, "roomcount") +
        " " +
        lbl +
        "</span> · <span>" +
        formatProperty(units[i].livingsurface, "livingsurface") +
        "</span></div>";
      li += "</div>";
      li += "</div>";
    ...
    $("#unit-list li[data-id]")
      .mouseenter(onMouseOverlist)
      .mouseleave(onMouseOutlist)
      .click(function (e) {
    ...
    for (var p in SALE_STATUS) {
      $e.append(
        $("<option>", { value: SALE_STATUS[p] }).text(
          SALE_STATUS[p].capitalize()
        )
      );
    }
    ...
    step = 5;
    maxLivingsurface = maxLivingsurface
      ? Math.ceil(maxLivingsurface / step) * step
      : 0;
    minLivingsurface = minLivingsurface
      ? Math.floor(minLivingsurface / step) * step
      : 0;
    if (
      !setSlider(sliderLivingsurface, minLivingsurface, maxLivingsurface, step)
    )
      numFilters--;
    ...
    if (sliderLivingsurface.noUiSlider) {
      sliderLivingsurface.noUiSlider.on("update", function (values, handle) {
        $("#unit-filter #livingsurface-from").html(
          Math.round(values[0]) + "m&#178;"
        );
        $("#unit-filter #livingsurface-to").html(
          Math.round(values[1]) + "m&#178;"
        );
      });
    ...
    for (var p in select) {
      ...
      if (select[p].length > 1) {
        ...
        for (i = 0; i < l; i++) {
          var t = select[p][i];
          if (p == "roomcount")
            t = select[p][i] + (select[p][i] > 1 ? " of meer" : " kamer");
  ...
  function showTab(tabElement, targetSelector) {
    ...
    if (!firstrun)
      $tab.find(".unit-content").css({ opacity: 0 }).animate({ opacity: 1 });
    return $tab;
  }
  ...
  function log(key, value) {
    ...
    return;
    //TODO: enable log when going live
...
//   }
  ...
  function filterBlock() {
    ...
    var livingsurfaces = sliderLivingsurface.noUiSlider
      ? sliderLivingsurface.noUiSlider.get()
      : [0, 0];
      ...
      switch (name) {
        case "price-from":
          ...
          b =
            ...
            (sliderPrice.noUiSlider
              ? sliderPrice.noUiSlider.options.range.min[0]
              : 0);
          if (b) {
            hasFilter = b;
            hasPriceFilter = b;
        ...
        case "price-to":
          ...
          b =
            val !=
            (sliderPrice.noUiSlider
              ? sliderPrice.noUiSlider.options.range.max[0]
              : 0);
          if (b) {
            hasFilter = b;
            hasPriceFilter = b;
        ...
        case "livingsurface-from":
          ...
          if (!hasFilter)
            hasFilter =
              val !=
              (sliderLivingsurface.noUiSlider
                ? sliderLivingsurface.noUiSlider.options.range.min[0]
                : 0);
          break;
        case "livingsurface-to":
          ...
          if (!hasFilter)
            hasFilter =
              val !=
              (sliderLivingsurface.noUiSlider
                ? sliderLivingsurface.noUiSlider.options.range.max[0]
                : 0);
          break;
        default:
        ...
        while (j--) {
          ...
          switch (name) {
            ...
            case "livingsurface-to":
              ...
              break;
            case "price-from":
              if (results[j].price == 0) include = true;
              //include = !hasPriceFilter;
    ...
    if (hasFilter) {
      ...
      if (i == 0) {
        ...
        while (i--) {
          ...
          for (var n in selectboxes) {
            if (
              !selectboxes[n].hasValue &&
              selectboxes[n].values.indexOf(String(results[i][n])) == -1
            )
              selectboxes[n].values.push(String(results[i][n]));
          }

          $(
            '.unit-selector g[id="' + buttonPrefixSvg + results[i].id + '"]'
          ).addClass("show");
          $('#unit-list li[data-id="' + results[i].id + '"]').removeClass(
            "hide"
          );
    ...
    switch (prop) {
      ...
      case "price_to":
        if ($.isNumeric(val) && val > 0)
          val =
            String(Math.floor(val)).formatCurrency(true) +
            " <span>v.o.n.</span>";
  ...
  function selectUnit(id) {
    ...
    $('.unit-selector g[id^="' + buttonPrefixSvg + '"].active').removeClass(
      "active"
    );
    ...
    for (var i = 0; i < numUnits; i++) {
      ...
      if (units[i].active) {
        ...
        $("#unit-info .unit-status").attr(
          "data-status",
          formatClassName(units[i].status)
        );
        ...
        $(
          '.unit-selector g[id^="' + buttonPrefixSvg + activeUnitId + '"]'
        ).addClass("active");
      }
    }
  ...
  function onMouseOverSVG(event) {
    ...
    $(
      '#unit-list li[data-id="' + getSVGUnitId(event.currentTarget) + '"]'
    ).addClass("hover");
